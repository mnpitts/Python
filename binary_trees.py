...
class SinglyLinkedList:
    ...
    def recursive_find(self, data):
        ...
        Returns:
            (SinglyLinkedListNode): the node containing the data.
        """
        #use a recursive function to search through the list
        def my_step(current):
            if current == None:						#Base case 1: dead end :(
                raise ValueError('The value could not be found.')
            elif current.value == data:				#Base case 2: WE FOUND IT!
                return(current)
            else:									#Iterate to the next node
                return(my_step(current.next))
        return (my_step(self.head))
...
class BST:
    ...
    def insert(self, data):
        ...
        def my_step(data, current_node):
            if current_node == None:					#Add the node to the tree
                current_node = BSTNode(data)
                return(current_node)
            elif current_node.value == data:			#Data is already in the tree
                raise ValueError('The value is already in the tree.')
            elif data < current_node.value:				#Move to the left
                current_node.left=my_step(data, current_node.left)
                current_node.left.prev = current_node
                return(current_node)
            elif data > current_node.value:				#Move to the right
    ...
    def remove(self, data):
        ...
        #Define a recursive function to remove a node
        def remove_from_tree(my_node):
            ...
            elif my_node.left != None and my_node.right != None:
                pred = my_node.left
            ...
            #The target has one child
                ...
                #The target is to the left of its parent
                elif my_node.prev.left == my_node:
                    if my_node.left != None:
...
def prob4():
    ...
    for n in num_list:
        ...
        my_single = SinglyLinkedList()
        my_binary = BST()
        my_avl = AVL()
        
        #time building a list
    ...
    axes[0].loglog(num_list, build_list,'b-o', basex=2, basey=10, label = 'List Build Times')
    axes[0].loglog(num_list, build_binary,'g-o', basex=2, basey=10, label = 'BST Build Times')                          #Graph all of the build times on one axes
    axes[0].loglog(num_list, build_avl,'r-o', basex=2, basey=10, label = 'AVL Build Times')
    axes[0].legend()
    axes[0].set_title('Build Execution Times')
    axes[0].set_xlabel('n')
    axes[0].set_ylabel('Seconds')
    axes[1].loglog(num_list, find_list,'b-o', basex=2, basey=10, label = 'List Find Times')
    axes[1].loglog(num_list, find_binary,'g-o', basex=2, basey=10, label = 'BST Find Times')                      #Graph all of the find times on another axes
    axes[1].loglog(num_list, find_avl,'r-o', basex=2, basey=10, label = 'AVL Find Times')
    axes[1].legend()
